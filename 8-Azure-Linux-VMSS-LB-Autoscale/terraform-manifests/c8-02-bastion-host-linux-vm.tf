locals {
  bastion_vm_custom_data = <<CUSTOM_DATA
  #!/bin/sh
  #sudo apt update -y
  ...
  sudo echo "Welcome to Culak Azure Web -  Bastion Host - VM Hostname: $(hostname)" > /var/www/html/index.html
  CUSTOM_DATA
}
...
resource "azurerm_linux_virtual_machine" "bastion_linux_vm" {
  ...
  custom_data = base64encode(local.bastion_vm_custom_data)
}
