resource "null_resource" "null_ssh_key_bastion_host" {
  ...
  provisioner "file" {
    source = "${path.module}/../ssh-keys/terraform-azure-bastion-linux-vm.pem"
    destination = "/tmp/terraform-azure-bastion-linux-vm.pem"
  }
  provisioner "remote-exec" {
    inline = [
      "sudo chmod 400 /tmp/terraform-azure-bastion-linux-vm.pem"
    ]
  }
}